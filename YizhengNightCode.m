...
%F = S_cond + S_f_gen - S_conv(nighttime) + S_emi(nighttime)

M= N_flesh;
F = @(T_M)((k_f/dr)*4*pi*r_o^2*(T(N_flesh -1, c-1) - T_M)   + q_f_gen*(4/3)*pi*(r_o^3 - (r_o - dr/2)^3)  - h*4*pi*r_o^2*(T_M-T_infn) + alpha_emi*(5.67*10^-8)*4*pi*r_o^2*(T_skyn^4 - T_M^4));
...
plot(range, Temp_fin);
title('Plot of temperature vs radius at Night');
xlabel('Radius (m)');
...
T_s=295.148300077858; %Surface temperature base on graph (NIGHT)

q_in= (k_f/dr)*4*pi*r_o^2*(T(N_flesh -1, c-1) - T_s)   + q_f_gen*(4/3)*pi*(r_o^3 - (r_o - dr/2)^3)
q_out= h*4*pi*r_o^2*(T_s-T_infn) - (alpha_emi*(5.67*10^-8)*4*pi*r_o^2*(T_skyn^4 - T_s^4))
